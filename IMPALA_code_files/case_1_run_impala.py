...
register(
     id="case1env",
     entry_point="case_1_env:Case1Env",
     max_episode_steps=100,
)

env = gym.make('case1env')

# csv file to store average network utilizations
with open("utils_impala_case1.csv", mode='w', newline='') as file:
    ...
    def on_episode_end(
        ...
        **kwargs,
        ...
        if episode_info is not None:
            ...
            with open("utils_impala_case1.csv", mode='a', newline='') as file:
              writer = csv.writer(file)
              writer.writerow([episode.episode_id, network_utilization])


config = (ImpalaConfig()
          .training(lr=0.0003, gamma=0.999)
          .resources(num_gpus=0)
          .callbacks(MyIMPALACallbacks)
          .environment(env='case1env')
